...
use crate::configs::fishenv::FishenvConfig;
use crate::formatter::StringFormatter;

use serde::Deserialize;

/// Creates a module with the current fishenv rc
pub fn module<'a>(context: &'a Context) -> Option<Module<'a>> {
    let mut module = context.new_module("fishenv");
    let config = FishenvConfig::try_load(module.config);

    let state = match FishenvState::from_env(context) {
        Ok(s) => s,
        Err(e) => {
            log::warn!("{e}");
...
impl FromStr for FishenvState {
    type Err = Cow<'static, str>;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match serde_json::from_str::<RawFishenvState>(s) {
            Ok(raw) => Ok(FishenvState {
            ...
            Err(_) => FishenvState::from_lines(s),
        }
    }
}
...
impl FishenvState {
    fn is_rc_allowed(path: Option<&PathBuf>) -> Option<AllowStatus> {
        ...
        let deny_file =
            Context::expand_tilde(PathBuf::from_str("~/.local/share/fishenv/deny").unwrap())
                .as_path()
        ...
        let allow_file =
            Context::expand_tilde(PathBuf::from_str("~/.local/share/fishenv/allow").unwrap())
                .as_path()
    ...
    fn from_env(context: &Context) -> Result<Self, Cow<'static, str>> {
        ...
        let loaded_rc_path = context
            .get_env("FISHENV_FILE")
            .map(|e| PathBuf::from_str(e.as_str()).unwrap());

        let allowed = FishenvState::is_rc_allowed(rc_path.as_ref());
        let loaded_allowed = FishenvState::is_rc_allowed(loaded_rc_path.as_ref());
    ...
    fn from_lines(s: &str) -> Result<Self, Cow<'static, str>> {
        ...
        if rc_path.is_none() || allowed.is_none() {
            return Err(Cow::from("unknown fishenv state"));
        }
...
mod tests {
    ...
    fn folder_without_rc_files_pre_2_33() {
        let renderer = ModuleRenderer::new("fishenv")
            .config(toml::toml! {
                [fishenv]
                disabled = false
            })
    ...
    fn folder_with_unloaded_rc_file_pre_2_33() -> io::Result<()> {
        ...
        assert_eq!(
            Some(format!("fishenv not loaded/allowed ")),
            renderer.collect()
        );
    ...
    fn folder_with_unloaded_rc_file() -> io::Result<()> {
        ...
        assert_eq!(
            Some("fishenv not loaded/allowed ".to_string()),
            renderer.collect()
        );
    ...
    fn folder_with_loaded_rc_file_pre_2_33() -> io::Result<()> {
        ...
        assert_eq!(Some(format!("fishenv loaded/allowed ")), renderer.collect());
    ...
    fn folder_with_loaded_rc_file() -> io::Result<()> {
        ...
        assert_eq!(
            Some("fishenv loaded/allowed ".to_string()),
            renderer.collect()
        );
    ...
    fn folder_with_loaded_and_denied_rc_file_pre_2_33() -> io::Result<()> {
        ...
        assert_eq!(Some(format!("fishenv loaded/denied ")), renderer.collect());
    ...
    fn folder_with_loaded_and_not_allowed_rc_file() -> io::Result<()> {
        ...
        assert_eq!(
            Some(format!("fishenv loaded/not allowed ")),
            renderer.collect()
        );
    ...
    fn folder_with_loaded_and_denied_rc_file() -> io::Result<()> {
        ...
        assert_eq!(
            Some("fishenv loaded/denied ".to_string()),
            renderer.collect()
        );
    ...
    fn status_cmd_output_without_rc() -> String {
        String::from(
            r"\
fishenv exec path /usr/bin/fishenv
FISHENV_CONFIG /home/test/.config/fishenv
...
No .envrc or .env found",
    ...
    fn status_cmd_output_with_rc(
        ...
        let loaded = if loaded {
            ...
            Loaded watch: "../.local/share/fishenv/allow/abcd" - 2023-04-30T09:52:58-04:00
...
Found watch: "../.local/share/fishenv/allow/abcd" - 1969-12-31T19:00:00-05:00
Found RC allowed {state}
Found RC allowPath /home/test/.local/share/fishenv/allow/abcd
"#
        )
    }
    ...
    fn config_dir() -> &'static str {
        r"C:\\Users\\test\\AppData\\Local\\fishenv"
    }
    #[cfg(not(windows))]
    fn config_dir() -> &'static str {
        "/home/test/.config/fishenv"
    }
    #[cfg(windows)]
    fn self_path() -> &'static str {
        r"C:\\Program Files\\fishenv\\fishenv.exe"
    }
    #[cfg(not(windows))]
    fn self_path() -> &'static str {
        "/usr/bin/fishenv"
